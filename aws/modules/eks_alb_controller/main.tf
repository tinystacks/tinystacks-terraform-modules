    ...
    Statement = [
      {
        "Effect" : "Allow",
        "Principal" : {
          "Federated" : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/oidc.eks.${var.region}.amazonaws.com/id/${local.oidc_url[4]}"
        },
...
resource "kubernetes_service_account_v1" "lb_controller_sa" {
  metadata {
    labels = {
